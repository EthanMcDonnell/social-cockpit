...
import { sendPrivateReply } from "@/lib/instagram/endpoints/comments";
...
export async function POST(
  ...
  if (!text?.trim()) {
    return NextResponse.json({ error: "missing_param", message: "text is required" }, { status: 400 });
  }

  try {
    const result = await sendPrivateReply(params.id, text.trim());
